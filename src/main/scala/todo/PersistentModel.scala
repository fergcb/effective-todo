...
object PersistentModel extends Model:
  ...
  def create(task: Task): Id =
    val tasks = loadTasks().tasks.toMap
    val id = loadId().next
    val newTasks = tasks + (id -> task)
  ...
  def update(id: Id)(f: Task => Task): Option[Task] =
    val tasks = loadTasks().tasks.toMap
    val task = tasks.get(id).map(task => f.apply(task))
    if task.isDefined then
      saveTasks(Tasks(tasks + (id -> task.get)))
    task
  ...
  def tasks: Tasks =
    val tasks = loadTasks().tasks
    Tasks(tasks)

  def tasks(tag: Tag): Tasks =
    val tasks = loadTasks().tasks
      .filter((_, task) => task.tags.exists(t => t.equals(tag)))
    Tasks(tasks)
  ...
  def tags: Tags =
    val tasks = loadTasks().tasks
    val tags = tasks.flatMap((_, task) => task.tags).toSet.toList
    Tags(tags)
